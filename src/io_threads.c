...
void replAssignAffinityTid(client *c) {
    ...
    if (repl_affinity_next_tid <= 0 || repl_affinity_next_tid >= server.io_threads_num) {
...
int testOnlyComputeWriteThreadId(client *c) {
    ...
    if (getClientType(c) == CLIENT_TYPE_REPLICA &&
        server.repl_compression_thread_affinity &&
...
int trySendReadToIOThreads(client *c) {
    ...
    /* Replication stream decoding state lives on the main-thread read path. */
    if (getClientType(c) == CLIENT_TYPE_PRIMARY && server.repl_stream_decoder) return C_ERR;
