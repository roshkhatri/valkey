...
TEST_F(IOThreadsTest, CompressionErrorSetsErrorFlagAndSkipsConnWrite) {
    ...
    ASSERT_EQ(c.repl_data->compression_error, 0);
    ASSERT_EQ(c.write_flags & WRITE_FLAGS_WRITE_ERROR, 0u);
    ...
    ASSERT_NE(c.write_flags & WRITE_FLAGS_WRITE_ERROR, 0u);
...
TEST_F(IOThreadsTest, CompressionRoundTrip) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        while (src_offset < compressed.size()) {
            ...
            if (produced > 0) {
                decompressed.insert(decompressed.end(), decomp_buf, decomp_buf + produced);
            }
            src_offset += consumed;
