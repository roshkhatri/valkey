 ...
 * the existing tests need no churn. Push-mode tests pass a pushConfig that
 * bundles the reader config with its feed cap (the production streamReaderConfig
 * has no cap field). */
typedef struct {
...
static streamReader *streamReaderCreatePush(pushConfig *pc) {
    streamReader *r = (streamReader *)zmalloc(sizeof(*r));
    if (streamReaderInitPush(r, &pc->cfg, pc->feed_cap) != 0) {
        zfree(r);
        return nullptr;
    }
    return r;
...
/* Helper: produce VCS-compressed bytes from a plaintext payload using streamWriter. */
...
static pushConfig makePushConfig(bool allow_passthrough, size_t push_feed_cap) {
    pushConfig pc = {makeReaderConfig(STREAM_KIND_RDB, allow_passthrough, 0), push_feed_cap};
    return pc;
}

/* 1. createPushRejectsZeroCap */
TEST_F(streamReaderPush, createPushRejectsZeroCap) {
    pushConfig cfg = makePushConfig(true, 0);
...
TEST_F(streamReaderPush, createPushWithValidCapSucceeds) {
    pushConfig cfg = makePushConfig(true, 4096);
    streamReader *r = streamReaderCreatePush(&cfg);
    ASSERT_NE(r, nullptr);
...
TEST_F(streamReaderPush, pushPassthroughDisallowedFails) {
    const char *payload = "not VCS data";
    size_t len = strlen(payload);

    pushConfig cfg = makePushConfig(false, 4096);
    streamReader *r = streamReaderCreatePush(&cfg);
    ASSERT_NE(r, nullptr);
...
TEST_F(streamReaderPush, pushCompressedRoundtripSingleChunk) {
    ...
    pushConfig cfg = makePushConfig(true, compressed_len + 1024);
    streamReader *r = streamReaderCreatePush(&cfg);
    ASSERT_NE(r, nullptr);
...
TEST_F(streamReaderPush, pushCompressedProbeAcrossFeeds) {
    ...
    sds compressed = pushTestCompress(payload, sizeof(payload));
    size_t compressed_len = sdslen(compressed);
    ASSERT_GE(compressed_len, (size_t)VCS_ENVELOPE_SIZE);
    ...
    ASSERT_EQ(streamReaderFeed(r, compressed + VCS_ENVELOPE_SIZE, compressed_len - VCS_ENVELOPE_SIZE), 0);
    streamReaderFeedEnd(r);
...
TEST_F(streamReaderPush, pushCompressedFrameDoneWithTrailingBytes) {
    ...
    pushConfig cfg = makePushConfig(true, total_len + 1024);
    streamReader *r = streamReaderCreatePush(&cfg);
    ASSERT_NE(r, nullptr);
...
TEST_F(streamReaderPush, pushNeedsInputTransitions) {
    ...
    /* After error → false */
    pushConfig cfg2 = makePushConfig(false, 4096);
    streamReader *r2 = streamReaderCreatePush(&cfg2);
    ASSERT_NE(r2, nullptr);
    ASSERT_EQ(streamReaderFeed(r2, "not VCS!", 8), 0);
...
TEST_F(streamReaderPush, pushFeedCapRejectsOverflow) {
    pushConfig cfg = makePushConfig(true, 1024);
    streamReader *r = streamReaderCreatePush(&cfg);
    ASSERT_NE(r, nullptr);
...
TEST_F(streamReaderPush, pushPassthroughReplaysPartialProbe) {
    /* Feed 3 bytes (not enough for VCS probe to decide), then 1 more byte
...
TEST_F(streamReaderPush, pushErrorIsSticky) {
    ...
    pushConfig cfg = makePushConfig(true, 8);
    streamReader *r = streamReaderCreatePush(&cfg);
    ASSERT_NE(r, nullptr);
