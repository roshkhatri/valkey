...
def argument_width(arg):
    """Return the number of argv entries consumed by a fixed argument shape."""
    if arg.type == "oneof":
        return 1
    if arg.type == "block":
        width = 1 if arg.desc.get("token") else 0
        for subarg in arg.subargs:
            width += argument_width(subarg)
        return width
    return 1
...
class Command(object):
    ...
    def struct_code(self):
        ...
        member_arg_index = self.infer_member_args()
        if member_arg_index:
